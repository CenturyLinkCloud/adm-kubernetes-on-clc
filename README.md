...
While it is possible to create the cluster with a public ip address and provision the system without VPN access, we do not recommend that approach at this time due to security concerns.
...
In the service definition the _spec_ must include _ports_ and _type_ definitions, for instance:
...
## Cluster management
The most widely used tool for managing a kubernetes cluster is the command-line utility _kubectl_. If you do not already have a copy of this binary on your administrative machine, you may run the script _install_kubectl.sh_ which will download it and install it in _/usr/bin/local_.

The script requires that the environment variable CLC_CLUSTER_NAME be defined

_install_kubectl.sh_ also writes a configuration file which will embed the necessary authentication certificates for the particular cluster. The configuration file is written to the ${CLC_CLUSTER_HOME}/kube directory
...
KubeUI is running at [https://${MASTER_IP}:6443/api/v1/proxy/namespaces/kube-system/services/kube-ui](https://${MASTER_IP}:6443/api/v1/proxy/namespaces/kube-system/services/kube-ui) kubernetes-dashboard is running at [https://${MASTER_IP}:6443/api/v1/proxy/namespaces/kube-system/services/kubernetes-dashboard](https://${MASTER_IP}:6443/api/v1/proxy/namespaces/kube-system/services/kubernetes-dashboard)

Note on Authentication to the UIs: The cluster is set up to use basic authentication for the user _admin_. Hitting the url at [https://${MASTER_IP}:6443](https://${MASTER_IP}:6443) will require accepting the self-signed certificate from the apiserver, and then presenting the admin password written to file at

> _${CLC_CLUSTER_HOME}/kube/admin_password.txt_

### Configuration files
Various configuration files are written into the home directory _CLC_CLUSTER_HOME_ under _.clc_kube/${CLC_CLUSTER_NAME}_ in several subdirectories. You can use these files to access the cluster from machines other than where you created the cluster from.
...
and then access urls like [http://127.0.0.1:8001/api/v1/proxy/namespaces/kube-system/services/kube-ui/](http://127.0.0.1:8001/api/v1/proxy/namespaces/kube-system/services/kube-ui/) without the need for client certificates in your browser.
